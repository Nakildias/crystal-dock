...
inline void drawIndicatorGlow(int cx, int cy, int radius, QColor color,
                              float intensity, QPainter* painter) {
  if (intensity <= 0.0f) return;
  painter->save();
  painter->setRenderHint(QPainter::Antialiasing, true);
  int glowRadius = radius * 3;
  QRadialGradient gradient(cx, cy, glowRadius);
  QColor glowColor = color;
  glowColor.setAlphaF(intensity * 0.6f);
  QColor transparent = color;
  transparent.setAlphaF(0.0f);
  gradient.setColorAt(0.0, glowColor);
  gradient.setColorAt(0.4, glowColor);
  gradient.setColorAt(1.0, transparent);
  painter->setPen(Qt::NoPen);
  painter->setBrush(gradient);
  painter->drawEllipse(QPointF(cx, cy), glowRadius, glowRadius);
  painter->restore();
}
