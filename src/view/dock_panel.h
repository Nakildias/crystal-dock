...
class DockPanel : public QWidget {
 ...
 private:
  ...
  // If fadeIn is true, the new item starts at opacity 0 for fade-in animation.
  bool addTask(const WindowInfo* task, bool fadeIn = false);
  ...
  bool isEnterAnimating_;  // true while enter zoom ramp-up is in progress
  bool isResizeAnimating_; // true while dock smoothly resizes for task add/remove
  double enterProgress_;   // 0.0 → 1.0, scales parabolic zoom during enter
  bool isShowingPopup_;
  bool layoutDone_ = false;

  // Slide-to-hide animation.
  std::unique_ptr<QTimer> slideTimer_;
  bool isSliding_;       // true during slide animation
  bool isSlidOut_;       // true when fully slid off-screen
  bool slideDirection_;  // true = sliding out, false = sliding in
  int slideOffset_;      // current pixel offset (0 = visible, minHeight_ = hidden)
  int slideTarget_;      // target offset
  int slideStart_;       // offset at animation start
  int slideStepCount_;   // current step
  int slideTotalSteps_;  // total steps
