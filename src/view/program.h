...
class Program : public QObject, public IconBasedDockItem {
 ...
 public:
  ...
  void updatePinnedStatus(bool pinned) override {
    pinned_ = pinned;
    if (pinAction_) pinAction_->setChecked(pinned_);
  }
  ...
  bool isBouncing() const override { return bouncing_; }
 ...
 private:
  ...
  QTimer breatheTimer_;
  float breathePhase_ = 0.0f;
  bool breatheActive_ = false;
