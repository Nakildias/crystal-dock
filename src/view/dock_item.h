 ...
 public:
  DockItem(DockPanel* parent, MultiDockModel* model, const QString& label,
      ...
      : parent_(parent), model_(model), label_(label), orientation_(orientation),
        ...
        left_(0), top_(0), minCenter_(0),
        startLeft_(0), startTop_(0), startSize_(minSize),
        endLeft_(0), endTop_(0), endSize_(minSize),
        currentStep_(0), numSteps_(0) {}
  virtual ~DockItem() {}
  ...
  virtual bool isBouncing() const { return false; }
  ...
  void nextAnimationStep() {
    ...
    if (currentStep_ <= numSteps_) {
      double t = static_cast<double>(currentStep_) / numSteps_;
      // easeInOutQuad easing function for smooth animation
      double eased = (t < 0.5) ? 2 * t * t : -1 + (4 - 2 * t) * t;
      left_ = startLeft_ + (endLeft_ - startLeft_) * eased;
      top_ = startTop_ + (endTop_ - startTop_) * eased;
      size_ = startSize_ + (endSize_ - startSize_) * eased;
    }
  }
